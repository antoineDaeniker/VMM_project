...
def get_models_by_config(cfg, gpus=[0], device0=DEVICE_CPU):
    ...
    for model in cfg.MODELS: # "model" must be a name of a model architecture
        ...
        exec(f'from . import {model_dict.DIR}') # DIR - filename holding the architecture
        params = model_dict.PARAMS if 'PARAMS' in model_dict else {}
        models[model] = eval(f'{model_dict.DIR}'+"."+f'{model_dict.ARCH}')(**params) # ARCH - must be network class name
        ...
        models[model] = th.nn.DataParallel(models[model], device_ids=gpus).to(device0)
